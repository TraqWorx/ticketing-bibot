...
export const renderTextWithLinks = (text: string): ReactNode[] => {
    ...
    matches.forEach(({ text: matchText, type }) => {
        const replacement = type === 'email'
            ? `<email>${matchText.trim()}</email>`
            : `<url>${matchText.trim()}</url>`;
    ...
    return parts.map((part, index): ReactNode => {
        if (part.startsWith('<email>') && part.endsWith('</email>')) {
            const email = part.slice(7, -8).trim();
            return (
                <Link key={index} href={`mailto:${email}`} color="blue.500" textDecoration="underline" wordBreak="break-all">
                    {email}
                </Link>
            );
        } else if (part.startsWith('<url>') && part.endsWith('</url>')) {
            const url = part.slice(5, -6).trim();
            let href = url;
            if (!href.startsWith('http://') && !href.startsWith('https://')) {
                href = `https://${href}`;
            }
            return (
                <Link key={index} href={href} target="_blank" rel="noopener noreferrer" color="blue.500" textDecoration="underline" wordBreak="break-all">
                    {url}
                </Link>
            );
        }
        // Rimuovi spazi vuoti attorno ai componenti link
